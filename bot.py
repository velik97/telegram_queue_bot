...
WEBHOOK_LISTEN = '78.155.206.129'  # На некоторых серверах придется указывать такой же IP, что и выше
...
def how_long_to_wait(message):
	...
	with DataBaseExecuter(config.db_host) as db:
		...
		bot.send_message(message.chat.id, dialogs.predict_respond(str(int(round(time.seconds / 60)))))
